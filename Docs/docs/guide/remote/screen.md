...
查看被控设备的画面，调分辨率、画质和 EDID。画面走 HDMI 捕获，BIOS、蓝屏、安全模式都能看见——不依赖操作系统。
...
| 提示 | 原因 |
|------|------|
| "远程画面未连接" | HDMI 没插或被控设备关机 |
| "主机休眠中" | 主机休眠或 HDMI 信号中断 |
| "信号协商中…" | HDMI 刚插入或分辨率切换，正在协商视频参数，稍候自动恢复 |
...
EDID 告诉被控设备"我是什么样的显示器"——支持什么分辨率、刷新率。下拉列表列出所有可用配置：
...
| 编码 | 说明 |
|------|------|
| H.264（默认） | 兼容性最好，所有浏览器都支持 |
| H.265（HEVC） | 同画质码率约省 30%，需要浏览器支持 H.265 |

- 未接入 HDMI 信号时不可切换。
- 出流中切换 → 短暂黑屏，随后自动重新协商；未出流时只保存设置，下次启动生效。
- 浏览器不支持所选编码 → 无法出流，切回 H.264 恢复。
...
| 按钮 | 作用 |
|------|------|
| 重新连接 | 断开当前视频连接再重建。画面卡顿或花屏时试试 |
| 拔出显示器 / 插入显示器 | 模拟显示器物理拔插，让主机重新识别显示器。主机"认不出"显示器或不输出画面时用 |

> 拔出后主机表现为显示器被拔掉；点"插入显示器"恢复。HDMI 热插拔检测为事件驱动，状态提示更及时。
...
点"删除自定义 EDID"确认后删除，画面菜单的 EDID 列表不再出现 custom 选项。EDID 正在使用中时先切换到其他 EDID 再删除。
...
| 字段 | 说明 |
|------|------|
| 制造商 | 3 个字母的制造商标识（如 `DEL`），自动转大写 |
| 显示器名称 | 操作系统里显示的显示器名称（可打印 ASCII） |
| 序列号字符串 | 显示器序列号字符串（可打印 ASCII） |
| 产品代码 | 数字产品代码，0 表示保留原值 |

留空的字段保留原值。保存后主机读取 EDID 时即显示修改后的信息——适合让主机把设备识别成指定显示器（多屏管理、采集/投屏软件按名称识别等场景）。

## 全屏

两种全屏模式（画面菜单的全屏按键也可切换）：

| 模式 | 快捷键 | 效果 |
|------|--------|------|
| 工具栏全屏 | `F11` | 画面铺满整个屏幕，菜单栏仍显示，功能随时可点 |
| 画面全屏 | `Ctrl+F11` | 隐藏菜单栏，只留画面 |

- 未全屏时按快捷键进入对应模式，再按同一快捷键退出。
- 已在全屏时按另一个快捷键，直接切换到另一种模式。
- 按 `Esc` 也可退出全屏。
