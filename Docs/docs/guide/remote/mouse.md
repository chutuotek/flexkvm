# 鼠标

控制被控设备的鼠标指针。支持绝对和相对两种模式，可调灵敏度和滚轮方向。

点菜单栏中的**鼠标**按钮进入鼠标菜单。

## 鼠标状态

菜单栏中的鼠标图标反映状态：

| 图标 | 含义 |
|------|------|
| ![](../icon/mouse/mouse.svg) | 已连接 |
| ![](../icon/mouse/mouse-off.svg) | 未连接或被关闭 |

## 鼠标菜单

点鼠标图标弹出菜单。

![鼠标菜单](./images/mouse/mouse.webp)

### 状态

| 状态 | 说明 |
|------|------|
| 已连接 | 正常控制 |
| 未连接 | 没连上，重新插拔 USB 或刷新页面 |
| 未使能 | 手动关了，点开启恢复 |

### 防休止

开启后鼠标每隔一段时间自动微移，防止主机因检测不到鼠标活动而休眠。网页开着或关了都会执行。不需要就关。

### 模式

**绝对模式（默认）**：画面点到哪里，主机鼠标就在哪里。

- ✅ 移动精准，无需点击画面即可控制
- ✅ 不受鼠标 DPI 影响
- ❌ 仅适合单屏幕
- ❌ 部分游戏可能不兼容

![绝对模式](./images/mouse/mouse_abs.webp)

**鼠标隐藏**：开启后鼠标移入画面自动隐藏，避免出现两个指针。

**相对模式**：像操作本地电脑一样移动主机鼠标。

- ✅ 支持多屏幕
- ✅ 兼容游戏等需要原始鼠标输入的应用
- ❌ 需要先点击画面进入控制
- ❌ 精准度受鼠标 DPI 影响

![相对模式](./images/mouse/mouse_rel.webp)

操作：点远程画面进入控制（浏览器请求锁定鼠标指针）→ 移动鼠标 → 按 `Esc` 退出。

> 非全屏相对模式下 `Esc` 被占用。需要发送 `Esc` 给主机？→ [Esc 映射](./keyboard.md#esc-映射)。
> 全屏相对模式下 `Esc` 正常工作，长按后会退出全屏。重新全屏按 `F11`。

**灵敏度**（仅相对模式）：调节范围 0.2~3.0，步进 0.2。1.0 正常速度。

**滚动速率**：调节范围 0.5~3.0，步进 0.5。1.0 正常速度。

**滚轮反转**：开启后滚轮方向反转。

**移动节流**：默认开启，限制鼠标移动事件的上报频率（默认 125 Hz，即每 8 毫秒合并一次），用"节流频率"（10~500 Hz）调节。高回报率鼠标（如 1000 Hz 游戏鼠标）下可明显降低浏览器与 USB 负载；关闭后移动数据全量上报，跟手度最高。

### 关闭与开启

点"关闭"→ 状态变为未使能。点"开启"恢复。

> 开关鼠标时 USB 短暂断开重连，键盘、磁盘挂载和音频也会短暂失效后恢复。

---

## 常见问题

**相对模式下鼠标出不来？** → 按 `Esc` 退出。

**绝对模式下位置不准？** → 检查主机是不是多屏幕。绝对模式只适合单屏。

---

[:octicons-arrow-left-24: 返回用户指南](../index.md)
