...
![2FA设置](../security/account/images/2fa-gen.webp)
...
SSH 登录不需要 2FA 验证码，仅需账号密码。如果需要更严格的 SSH 安全控制，可以关闭 SSH 服务，详见 [SSH](../security/ssh.md)。

### 账户管理

定期修改密码、管理多用户权限，详见 [账号管理](../security/account/account.md)。
...
SSH 详细配置和可用命令见 [SSH 远程访问](../security/ssh.md)。
...
进入 Web 界面 → 设置 → 维护 → 审计，可以查看和导出审计记录。详见 [审计日志](../security/audit.md)。
