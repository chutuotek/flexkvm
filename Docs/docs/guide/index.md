# 用户指南

欢迎使用 FlexKVM 用户指南！本章节将帮助您了解 FlexKVM 的各种功能。

---

## 目录

### 设备

- [包装清单](device/packet.md) - 开箱物品清单
- [接口说明](device/interface.md) - 产品接口规格与指示灯状态
- [物理与电气特性](device/characteristics.md) - 电源、功耗、尺寸与环境参数
- [背板](device/backplate.md) - 磁吸背板、3.5mm机柜背板
- [拓展](device/extensibility.md) - POE分离器等拓展配件

### 远程控制

- [屏幕显示](remote/screen.md) - WebRTC 视频流、画质调节、HDMI 状态
- [鼠标](remote/mouse.md) - 鼠标模式切换、指针锁定、灵敏度设置
- [键盘](remote/keyboard.md) - 键盘启用/禁用、虚拟键盘、文本粘贴
- [截图](remote/screenshot.md) - 截取画面
- [录像](remote/record.md) - 录制视频

### 账号

- [登录](account/login.md) - 用户登录
- [账号管理](account/account.md) - 密码修改、两步验证 (2FA)

### 网络

- [以太网](network/eth.md) - DHCP/静态 IP、网关 DNS 配置
- [WiFi](network/wifi.md) - WiFi 连接、保存网络管理
- [热点模式](network/ap.md) - 创建 WiFi 热点、频段与加密设置
- [热点配网](network/ap_province.md) - 热点配网功能

### 系统

- [USB 配置](system/usb.md) - 自定义 USB 设备
- [HTTPS 配置](system/https.md) - SSL 证书管理、自定义证书上传
- [SSH 远程访问](system/ssh.md) - SSH 服务启用与配置
- [OLED 显示](system/oled.md) - 屏幕亮度、息屏时间设置
- [时间设置](system/time.md) - 时区配置、NTP 同步
- [EDID 配置](system/edid.md) - 自定义 EDID 文件管理

### 高级功能

- [硬盘挂载](advanced/disk.md) - TF 卡管理、USB Mass Storage 导出
- [ATX 电源控制](advanced/atx.md) - 远程开关机、电源状态监控
- [网络唤醒 (WoL)](advanced/wakeonlan.md) - 远程唤醒目标主机

### 主题与语言

- [主题](theme/theme.md) - 界面主题切换
- [语言](theme/language.md) - 界面语言切换

### 升级

- [离线升级](upgrade/ota_offline.md) - 离线升级
- [在线升级](upgrade/ota_online.md) - 在线升级

### 维护

- [日志](maintenance/log.md) - 系统日志
- [审计](maintenance/audit.md) - 审计日志
- [重启与重置](maintenance/power_reset.md) - 重启设备、恢复出厂设置

### 应用中心

- [Tailscale](app/tailscale.md) - VPN 远程访问、穿透 NAT
