...
FlexKVM 的百兆有线网口提供稳定可靠的有线网络连接，与 WiFi 互为备份，一条链路断开时另一条自动接管。支持 DHCP 和静态 IP 两种方式获取 IP 地址。
...
> 提示：网络配置修改后立即生效，无需重启设备。但如果配置错误导致无法访问，可通过其他网络接口（如 WiFi）重新连接修改。
