...
FlexKVM becomes a DHCP server — connect a computer directly with a single Ethernet cable, and the computer automatically gets an IP to access the management interface. No router needed, no dependency on on-site network. Long-press Button B for 3–6 seconds to toggle on.
...
Long-press **Button B** has three stages. Release at the first stage (3–6s) to toggle Direct Mode:

| Current mode | Action | Result | OLED preview | Wait time |
|:-----------:|--------|--------|:----------:|:---------:|
| Ethernet client | Hold B 3s, release when OLED shows "ETH SERVER" | DHCP client off, Direct Mode on | ETH SERVER text | ~5s |
| Direct Mode | Hold B 3s, release when OLED shows "ETH CLIENT" | Direct Mode off, DHCP client restored | ETH CLIENT text | ~5s |

> Keeping the button held: 6–9s enters WiFi/hotspot toggle preview, 9s+ enters return-to-home preview. See [Physical Buttons](../interaction/button.md).
...
> If still unable to connect, use [Provisioning Mode](./provision.md) (long-press Button A 3–6s) to recover.
