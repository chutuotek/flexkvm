...
> 直连模式与普通有线网络模式互斥——开启直连模式会自动关闭 DHCP 客户端模式。
...
配合 [AP 热点](./ap.md)，就是**有线直连 + WiFi 热点**双保险——笔记本走网线、手机走 WiFi，任一路都能在没有路由器的现场管理设备。
...
> 直连模式与普通有线网络模式共用网口，OLED 图标不同以区分当前模式。
...
|:--------:|:----:|:--------:|:--------:|:--------:|
| 有线网络客户端 | 长按 B 1~3 秒，OLED 切到直连专用图标后松开 | 关 DHCP 客户端，开直连模式 | 直连专用图标 | 约 5 秒 |
| 直连模式 | 长按 B 1~3 秒，OLED 切回有线网络图标后松开 | 关直连模式，恢复 DHCP 客户端 | 有线网络图标 | 约 5 秒 |

> 继续长按不松手：3~5 秒进入 WiFi/热点切换预览，5 秒后进入返回首页预览。详见[物理按键](../interaction/button.md)。

## 软件配置

点击有线网络卡片齿轮图标 → **直连模式**标签。

![直连模式设置](./images/eth/setting_eth_server.webp)

### 启用与禁用

打开「直连模式开启」开关，设置网段号（0~254），点保存即可生效。
...
> 直连模式的网段不能与 AP 热点网段相同（两个接口不能同时是 `192.168.x.1`），网段冲突时开关无法保存，换一个网段号即可。

> 改网段号后连不上，普通有线网络模式会自动尝试恢复 DHCP，电脑端记得重新获取 IP。
>
> 如果仍然无法连接，使用[配网模式](./provision.md)（长按按键 A 1~3 秒）恢复。
...
| 网段号保存不了 | 与 AP 热点网段冲突 | 换一个网段号（两者不能同为 `192.168.x.1`） |
| 直连模式和普通有线网络反复切换失败 | 路由表残留 | 等待约 10 秒自动恢复，或重启设备 |
