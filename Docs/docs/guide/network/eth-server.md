...
|:--------:|:----:|:--------:|:--------:|:--------:|
| 以太网客户端 | 长按 B 1~3 秒，OLED 切到直连专用图标后松开 | 关 DHCP 客户端，开直连模式 | 直连专用图标 | 约 5 秒 |
| 直连模式 | 长按 B 1~3 秒，OLED 切回以太网图标后松开 | 关直连模式，恢复 DHCP 客户端 | 以太网图标 | 约 5 秒 |

> 继续长按不松手：3~5 秒进入 WiFi/热点切换预览，5 秒后进入返回首页预览。详见[物理按键](../interaction/button.md)。

## 软件配置

进 Web 界面 → 设置 → **网络** → 点击有线网络卡片齿轮图标 → **直连模式**标签。
...
> 改网段号后连不上，普通以太网模式会自动尝试恢复 DHCP，电脑端记得重新获取 IP。
>
