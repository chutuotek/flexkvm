...
配网模式是一个**临时** WiFi 热点——设备没联网、配置改错了连不上，长按按键 A 1~3 秒进去，配好网络后保存退出。和[AP 热点](./ap.md)不同：配网模式配完自动关，AP 热点持续运行。

> 入口：长按**按键 A** 1~3 秒进入配网模式，连上 OLED 显示的热点，浏览器打开 `http://192.168.10.1`
...
| 用途 | 首次配网 / 故障恢复 | 日常开关热点 |
| 退出方式 | 配网页面点保存；或长按按键 A / B 1 秒以上强制退出（不保存） | 再长按按键 B 3~5 秒 |
...
松开后进入等待界面，约 12 秒后 OLED 显示热点名称、随机密码（8 位数字）和 IP 地址：
...
**手机**：WiFi 设置里找到 FlexKVM 热点 → 输入 OLED 显示的密码 → 系统自动弹出浏览器门户页，点击即可进入配网页面。没弹出来就手动访问 `http://192.168.10.1`。Android 用户也可在通知栏点"登录网络"。

**电脑**：连上热点 → 浏览器访问 `http://192.168.10.1`。
...
进入配网页面，网络设置与正常 Web 界面一致：

> 设备时间错乱时（如 RTC 掉电后），配网页面连接后自动弹出校准提示，点"确认"将设备时间校准为当前时间。校准不要求登录，也不影响配网流程。

### 有线网络

DHCP / 静态 IP、自定义 MAC 等。详见 [有线网络](./eth.md)。
