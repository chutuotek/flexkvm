...
**即插即用的硬件级带外管理设备。** 将它接入被控设备的 HDMI 和 USB，你就能通过浏览器或 Tailscale 网络，随时随地查看屏幕、控制键鼠、开关电源、挂载 ISO —— 即使被控系统宕机、无显卡驱动、甚至未安装操作系统。
...
| 现场没有网络，没法远程连设备 | 直连模式（DHCP 服务器）+ WiFi 6 热点双通道，无需路由器，手机或电脑任选其一接入 |
...
- **:material-robot: AI 远程控制**
    ...
    内置 Agent 接口与 MCP 服务，Claude、OpenClaw 等 AI 客户端可自动截屏、控制键鼠，实现无人值守运维。
...
- **:material-lan-connect: 无网控制**
    ...
    没路由器，没 WiFi 也不怕。直连模式（DHCP 服务器）+ WiFi 6 自建热点，双通道零依赖接入，装机、调 BIOS、排查故障照干不误。
...
- **:material-magnet-on: 多场景部署**
    ...
    磁吸背板/35mm 机柜夹/自定义背板 多种方式固定，适配桌面、机柜、工业现场等场景。
