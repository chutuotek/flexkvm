...
| 现场没有网络，没法远程连设备 | 自建 WiFi 6 热点，手机/笔记本直连即可操作，不用等人拉网线 |
...
- **:material-headset: 双向音频**
    ...
    模拟 USB 音频设备，被控主机的系统声音实时传回浏览器，操作端麦克风输出到被控主机。故障诊断时一边看屏幕一边听蜂鸣声，就像坐在机器旁边。
...
- **:material-wifi-arrow-left-right: 双网冗余**
    ...
    5GHz 频段专传视频、低延迟不卡顿，百兆有线守住底线。一条链路断开另一条自动接管——拔了网线画面不黑、键鼠不断。

- **:material-access-point-network: WiFi 6 自建热点**

    机柜没拉网、客户现场不准接内网？FlexKVM 自己就是 WiFi 6 热点。手机连上就能完成装机、调 BIOS、排查故障。部署零依赖。
...
- **:material-sd: 外部存储**
    ...
    支持 TF 卡作为外部存储，即插即用，兼容 FAT32、exFAT、ext4 等多种文件系统。
...
| 音频 | USB 音频模拟（播放 + 录音），WebRTC 双向传输 |
| 网络 | 百兆有线 + WiFi 6 双频（2.4G / 5G）+ WiFi 6 热点 |
| 工作温度 | 0°C ~ 70°C |
