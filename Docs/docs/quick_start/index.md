...
| 网线 | 有线联网 | 使用热点配网则不需要 |
| 已启动的被控设备（电脑/服务器） | 被远程管理的目标设备 | 需具备 HDMI 输出和 USB 接口；若为 Mini HDMI / Micro HDMI 等非标准接口，需自备转接头或转换线 |
...
| 快闪 | 约 4 次/秒 | 紧急事件(ota升级，恢复出厂模式等) | ⚠️ 快速上手阶段不会出现这种状态 |
...
> 排查：如果 OLED 无显示或 🔴 电源灯不亮，可能是电源适配器供电不足，请使用 5V/1A 以上的适配器。
>
...
> 排查：如果 🟢 状态灯仍为快闪，请检查被控设备是否已正常开机、USB 线材是否支持数据传输（包装内三根 USB Type-C 线均支持数据传输；若使用自备线材，请确认其为数据线而非仅充电线，部分仅充电线无法传输键鼠信号）。
...
> 设备在没有联网的情况下，🔴 **警告灯 [1]** 会**慢闪**（约 1 次/秒），OLED 第二行会显示"NO NETWORK !"。
...
- 一般 **5 秒内**自动获取 IP 地址，OLED 显示 IP（如 `E192.168.1.100`，前缀 `E` 表示 Ethernet 有线网络；在浏览器访问时只需输入数字部分，如 `192.168.1.100`），🔴 **警告灯 [1]** 熄灭。
...
可以看到，设备已经获取到了IP地址，并且 🔴 警告灯已经熄灭。

![设备显示wifi](./images/oled_wifi_connected.webp)

> ✅ **完成确认**：OLED 显示设备 IP 地址，🔴 警告灯 [1] 熄灭。
