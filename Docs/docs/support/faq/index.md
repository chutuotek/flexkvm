...
远程软件依赖操作系统运行。系统蓝屏、未启动、或无显卡驱动时远程软件无法工作。FlexKVM 在**硬件层面**捕获视频信号，不依赖被控系统的任何软件状态。
...
配网模式是 FlexKVM 启动的一个临时 WiFi 热点，用于首次配置网络或在网络配置错误导致无法访问时恢复连接。长按**按键 A** 1~3 秒即可进入。详见 [配网模式](../../guide/network/provision.md)。
...
推荐使用 Chrome、Edge 等现代浏览器访问 Web 界面。其他浏览器可能存在兼容性问题（如画面显示异常）。
